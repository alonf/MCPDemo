...
public partial class McpServerProcessToolType
{
    ...
    private ProcessesInfoResult GetProcesses(Func<DiagnosticsProcess[]> getProcessesFunc, int? pageNumber, int? pageSize, string sourceDescription)
    {
        ...
        try
        {
            _logger.LogDebug("Retrieving processes for {Source}", sourceDescription);
            ...
            if (startIndex < processes.Length)
            {
            ...
            _logger.LogInformation("Successfully retrieved {Count} processes for {Source}", result.Processes.Count, sourceDescription);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error retrieving processes for {Source}", sourceDescription);
